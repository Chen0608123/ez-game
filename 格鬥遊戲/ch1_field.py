...
def main(selected_character):
    ...
    if selected_character == "角色1":
        ...
        ch2 = {
            ...
            "hitbox_rect":pygame.Rect(400,300,60,150),  # 獲取 hitbox 的矩形範圍
            "is_broken": False,  # 是否處於破防狀態
            "break_start_time": 0,  # 破防開始時間
    ...
    def handle_special2_move(ch1, ch2, keys, key_maps, screen, width):
        ...
        if ch1["energy"] >= 30 and not ch1.get("special2_active", False):
            is_right_facing = ch1["facing"] == 1
            correct_keys = (keys[key_maps["medium"]] and 
            keys[key_maps["defense"]] and (keys[key_maps["right"]] if is_right_facing else keys[key_maps["left"]]))
            ...
            if correct_keys:
                ...
                if is_right_facing:
                    special_hitbox = pygame.Rect(
                        ch1["hitbox_rect"].right,
                        ch1["hitbox_rect"].top - 20,  # 向上延伸20像素
                        ch1["hitbox_rect"].width + 20,  # 向右延伸20像素
                        ch1["hitbox_rect"].height + 40  # 上下各延伸20像素
                    )
                else:
                    special_hitbox = pygame.Rect(
                        ch1["hitbox_rect"].left - 20,  # 向左延伸20像素
                        ch1["hitbox_rect"].top - 20,  # 向上延伸20像素
                        ch1["hitbox_rect"].width + 20,  # 向左延伸20像素
                        ch1["hitbox_rect"].height + 40  # 上下各延伸20像素
                    )
                ...
                if special_hitbox.colliderect(ch2["hitbox_rect"]):
                    if ch2["is_blocking"]:
                        ...
                        # 添加擊飛效果
                        ch2["is_jumping"] = True
                        ch2["vertical_speed"] = -ch2["jump_speed"] * 1.25
                        # 水平擊退
    ...
    def draw_special2_move_ch2(screen, character):
        if character.get("special2_active", False):
            ...
            if character["spe2_x"] < -100 or character["spe2_x"] > width + 100:
                character["special2_active"] = False
    ...
    countdown_time = 99  # 倒數時間（秒）
     ...
     # === 主遊戲迴圈 ===
    while True:
        if opponent.get("show_special2_animation", False):
            draw_special2_animation(screen, opponent, width, height)
            pygame.display.update()
            clock.tick(120)
            # 動畫結束後立即判斷勝負
            if not opponent.get("show_special2_animation", False):
                if player["life"] <= 0:
        ...
        # 處理特殊技
        if player is ch1:
            handle_special_move(player, opponent, keys, player_keys, screen, width)
            handle_special2_move(player, opponent, keys, player_keys, screen, width)
        elif player is ch2:
        ...
        elif opponent is ch2:
            handle_special_move_ch2(opponent, player, keys, opponent_keys, screen, width)
            handle_special2_move_ch2(opponent, player, keys, opponent_keys, screen, width)
        ...
        # 繪製生命值條
        draw_health_bar_ch1(screen, player, 50, 20, 200, 20, GREEN)  # 玩家生命值條
        draw_health_bar_ch2(screen, opponent, 390, 20, 200, 20, GREEN)  # 對手生命值條
        ...
        # 繪製角色
        draw_character(screen, player)
        draw_character(screen, opponent)

        # 特殊技繪製
        if player is ch1:
            draw_special_move(screen, player)
            draw_special2_move(screen, player)
        elif player is ch2:
        ...
        if opponent is ch1:
            ...
            draw_special_move_ch2(screen, opponent)
            draw_special2_move_ch2(screen, opponent)

        # 用圖片顯示 hitbox
        screen.blit(player["hitbox_img_ch1"], player["hitbox_rect"].topleft)    # 玩家 2 的 hitbox 圖片
        screen.blit(opponent["hitbox_img_ch2"], opponent["hitbox_rect"].topleft)  # 玩家 1 的 hitbox 圖片
